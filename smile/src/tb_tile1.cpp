...
class DramMemoryPort : public MemoryPort {             // inherit 
public:
  explicit DramMemoryPort(Dram& dram) : dram_(dram) {} // construct (use ref Dram obj as input), init member dram_ with dram
  uint32_t read32(uint32_t addr) override {              // read 32b value at stored addr
    uint32_t value      = 0;
  ...
  void write32(uint32_t addr, uint32_t value) override { // write 32b value to store addr
...
struct DebuggerState {
  ...
  DebuggerState(Tile1& t, DramMemoryPort& d) : tile(t), dram_port(d) {
    reset();
  }
...
static void run_debugger(DebuggerState& state) {
  ...
  while (true) {
    ...
    if (cmd == "step") {
      ...
      if (iss >> count_token) { // get 2nd word of i/p (possibly a step count)
      ...
      for (uint32_t i = 0; i < count; ++i) {
        ...
        if (info.log_breakpoint_snapshot) {   // print detailed context view on 1st ebreak
          print_breakpoint_snapshot(state, info.thread, info.begin_pc, info.mcause);
        }
        if (info.executed_breakpoint_instr) { // pause on ebreak
          std::cout << "Software breakpoint executed at 0x"
                    << hex32(info.begin_pc) << std::endl;
          break;
        }
        if (info.program_exited) {            // detect clean program exit via ecall 93
          break;
        }
      }
...
int main(int argc, char *argv[]) {
  ...
  DebuggerState dbg(tile, dram_port);
  if (max_cycles > 0) {
