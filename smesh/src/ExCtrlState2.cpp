...
ExCtrlState2::ExCtrlState2(std::string /*name*/, IMPL_CTOR) {
  ...
  UPDATE(updateDecisionAndOutputs)
      ...
      .writes(performing_single_preload, performing_mul_pre,
  ...
  UPDATE(updateState)
      ...
      .reads(head_bits, about_to_fire_all_rows, c_address_rs2,
             current_dataflow)
      .reads(control_state, perform_single_preload_Q_)
      .writes(cmd_pop_count)
      .writes(control_state_D_, perform_single_preload_D_, perform_mul_pre_D_,
              perform_single_mul_D_)
      ...
      .writes(weight_double_bank_D_, weight_triple_bank_D_, row_left_D_,
              row_turn_D_)
...
void ExCtrlState2::updateDecisionAndOutputs() {
  const auto fsm_state = toFsmState(*control_state);
  const bool waiting = fsm_state == ExCtrlFsmState::WaitingForCmd;

  const bool accepting_config =
      waiting && head_val[0] == 1 && do_config == 1 &&
      matmul_in_progress == 0 && pending_completed_val == 0;
  const bool accepting_single_preload =
      waiting && !accepting_config && head_val[0] == 1 &&
      do_preloads[0] == 1 && head_val[1] == 1 &&
  ...
  const bool active_single_preload =
      fsm_state == ExCtrlFsmState::Compute &&
      perform_single_preload_Q_ == 1;
  const bool active_mul_pre =
      fsm_state == ExCtrlFsmState::Compute && perform_mul_pre_Q_ == 1;
  const bool active_single_mul =
      fsm_state == ExCtrlFsmState::Compute && perform_single_mul_Q_ == 1;

  performing_single_preload = bit(active_single_preload ||
                                   accepting_single_preload);
  ...
  computing = bit(active_single_preload || accepting_single_preload ||
                  active_mul_pre || active_single_mul);
  ...
  switch (fsm_state) {
    case ExCtrlFsmState::WaitingForCmd:
      ...
      if (accepting_config_ == 1) {
        ...
        if (type == ConfigKind::Execute) {
          ...
          if (!set_only_strides) {
            in_shift_D_ =
                static_cast<std::uint8_t>(unpackConfigExInShift(rs2));
            activation_D_ =
                static_cast<std::uint8_t>(unpackConfigExActivation(rs1));
            ...
            current_dataflow_D_ =
                static_cast<std::uint8_t>(unpackConfigExDataflow(rs1));
          }
        ...
        control_state_D_ =
            static_cast<std::uint8_t>(ExCtrlFsmState::Compute);
      }
      break;
    ...
    case ExCtrlFsmState::Compute:
      if (perform_single_preload_Q_ == 1 &&
        ...
        cmd_pop_count = 1;
        control_state_D_ =
            static_cast<std::uint8_t>(ExCtrlFsmState::WaitingForCmd);
