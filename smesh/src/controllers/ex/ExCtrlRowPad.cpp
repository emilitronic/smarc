// **********************************************************************
// smesh/src/controllers/ex/ExCtrlRowPad.cpp
// **********************************************************************
...
ExCtrlRowPad::ExCtrlRowPad(std::string /*name*/, IMPL_CTOR) {
  UPDATE(update)
      .reads(a_fire_counter, b_fire_counter, d_fire_counter,
             a_rows, b_rows, d_rows)
      .reads(a_cols, b_cols, d_cols,
             block_size)
      .writes(a_row_is_not_all_zeros, b_row_is_not_all_zeros, d_row_is_not_all_zeros,
...
void ExCtrlRowPad::update() {
  ...
  const bool a_real = a_counter < static_cast<std::uint32_t>(*a_rows); // is this a real A row?
