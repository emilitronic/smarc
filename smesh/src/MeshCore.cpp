...
void MeshCore::stepPreloadB(const MeshInputRow& b_top) {
  ...
  next_b[0] = b_top;                              // inject row at top
  for (std::size_t row = 1; row < kDim; ++row) {  // shift rows down
    next_b[row] = b_pipe_[row - 1];
  }

  b_pipe_ = next_b;
  weights_ = b_pipe_; // copy into weights
}

// WS compute movement LR/TB
void MeshCore::stepWsCompute(const MeshInputRow& a_left, const MeshAccumRow& psum_top) {
  InputGrid next_a{};
  AccumGrid next_psum{};
  MeshAccumRow next_bottom{};
  // move values and compute partial sums
  for (std::size_t row = 0; row < kDim; ++row) {
    for (std::size_t col = 0; col < kDim; ++col) {
      const auto a    = col == 0 ? a_left[row]   : a_pipe_[row][col - 1];    // LR move
      const auto psum = row == 0 ? psum_top[col] : psum_pipe_[row - 1][col]; // TB move 
