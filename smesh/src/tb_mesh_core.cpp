...
int main() {
  ...
  preload1.in_d = d1;
  preload1.control = controlRow(false);
  preload1.status = statusRow(smesh::MeshCoreStatus{2, false, true});
  core.step(preload1);

  smesh::MeshCoreIn preload2{};
  preload2.in_d = d2;
  preload2.control = controlRow(false);
  preload2.status = statusRow(smesh::MeshCoreStatus{3, false, true});
  core.step(preload2);

  bool ok = true;
  ok = ok && core.dPath()[0] == d1;
  ok = ok && core.dPath()[1] == smesh::MeshInputRow{};
  ok = ok && core.c2()[0] == d2;
  ok = ok && core.c2()[1] == d0;
  ...
  smesh::MeshInputRow d3{13, 14, 15, 16};
  smesh::MeshCoreIn compute0{};
  compute0.in_a = a0;
  compute0.in_d = d3;
  compute0.control = controlRow(true);
  compute0.status = statusRow(smesh::MeshCoreStatus{4, true, true});
  core.step(compute0);

  ok = ok && core.c1()[0] == d3;
  ok = ok && core.aPath()[0][0] == 1;
  ok = ok && core.aPath()[1][0] == 2;
  ok = ok && core.bPath()[0][0] == 9;
  ok = ok && core.controlPath()[0][0].prop;
  ok = ok && core.statusPath()[0][0].valid;
  ok = ok && core.statusPath()[0][0].in_id == 4;
